...
def state_list_to_bdd(
    ctx: SymbolicContext | BddVariableSet, states: list[BooleanSpace]
) -> Bdd:
    """
...
def function_eval(f: Bdd, state: BooleanSpace) -> Literal[0, 1] | None:
    """
    Evaluate a BDD function in the given state to an integer value. If the state
    is incomplete (i.e. it is a space), the function may not evaluate to an
    exact integer. In such case, `None` is returned.
    """
    if f.is_false():
        return 0
    if f.is_true():
