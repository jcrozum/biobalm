...
import balm.FVSpython3.FVS_localsearch_10_python as FVS10
...
def FVS(G, T_0=0.6, alpha=0.99, maxMvt_factor=5, maxFail=50, randomseed=None):
    ...
    if nx.is_weakly_connected(Gtemp):
        result = FVS_weakly_connected(
            Gtemp, T_0, alpha, maxMvt_factor, maxFail, randomseed
        )
    else:
...
def FVS_weakly_connected(G, T_0, alpha, maxMvt_factor, maxFail, randomseed):
    ...
    self_loops = [
        edge[0] for edge in G.edges() if edge[0] == edge[1]
    ...
    S_optimal_formatted = FVS10.FVS_local_search(
        G_formatted, T_0, alpha, maxMvt, maxFail, randomseed
    )
...
def FVS_disconnected(G, T_0, alpha, maxMvt_factor, maxFail, randomseed):
    ...
    wccs = (G.subgraph(c).copy() for c in nx.weakly_connected_components(G))
    # wccs = nx.weakly_connected_component_subgraphs(G)
    FVS = []
    for wcc in wccs:
        FVS_temp = FVS_weakly_connected(
            wcc, T_0, alpha, maxMvt_factor, maxFail, randomseed
        )
        FVS.extend(FVS_temp)
    return FVS
